...
| 3 | Run `blitzwing` — the mother operator mints a company-owned ENS subname on Sepolia (no Sepolia wallet needed for Dana) | She receives `host-abc123.blitzwing.eth` printed in the CLI success message |
| 4 | The operator writes resolver records: `blockIndices`, `hederaAccountId`, and model metadata on Sepolia | Her Hedera payout address is discoverable by anyone who resolves her ENS name |
| 5 | Client starts serving; heartbeats keep her in the live routing table | Liveness is tracked by orchestrator heartbeats (company renews the parent `.eth` name) |
...
| 8 | She opens **HashScan** and searches her account, or checks the HCS audit topic | She sees a consensus-timestamped log entry: `{requestId, ens_name: host-abc123.blitzwing.eth, amount, txId}` — independent proof she was paid for that specific request |
...
| 5 | Response comes back: completion text **plus a receipt/manifest** | The manifest lists exactly which ENS-named hosts (`mother.blitzwing.eth`, `host-abc123.blitzwing.eth`, ...) computed which layers, and the Hedera transaction ID for each payout |
